  ...
  return (
    <div>
      ...
      <p className="mt-2 text-[13px]">
        {outOfStock ? (
          <span className="font-medium text-clay">Out of stock</span>
        ) : product.stock <= 5 ? (
          <span className="font-medium text-clay">Only {product.stock} left in stock</span>
        ) : (
          <span className="font-medium text-moss">In stock</span>
        )}
      </p>
        ...
        <>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button onClick={handleAddToCart} className="btn-secondary flex-1">
              {added ? <Check size={16} /> : <ShoppingBag size={16} strokeWidth={1.75} />}
