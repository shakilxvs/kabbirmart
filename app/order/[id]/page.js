  ...
  return (
    <div className="container-page py-14 sm:py-20">
      <div className="mx-auto max-w-lg text-center">
        <CheckCircle2 size={44} strokeWidth={1.25} className="mx-auto text-moss" />
      ...
      <div className="mx-auto mt-10 max-w-lg rounded-2xl border border-line bg-surface p-6">
        <ul className="divide-y divide-line">
          {order.items.map((item, i) => (
            <li key={i} className="flex justify-between py-3 text-[13px]">
        ...
        <div className="mt-3 space-y-2 border-t border-line pt-4 text-[13px]">
          ...
          {order.codCharge > 0 && (
            <div className="flex justify-between text-ink-soft">
              ...
              <span className="text-ink">{formatBDT(order.codCharge)}</span>
            </div>
          )}
        </div>
